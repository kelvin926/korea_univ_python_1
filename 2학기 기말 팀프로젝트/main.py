...
'''
[to do list]
1. 교통 상황 예측 부분 코딩. [교통 상황을 CSV로 받는게 아니라, time함수를 이용해서, 출퇴근 시간 + 날씨(비 오는지)에 따라 교통 상황 예측.] (6/5)
2. 차량 운행 지수 코딩. [미세먼지 정도 + 날씨[비 오는지]] (6/5)
3. 전체 디버깅 + 중복 함수 제거 (6/5)
...
def f_rain(day,date_option):
    rain = open('korea_univ_python_1/2학기 기말 팀프로젝트/csv_file/rain.csv', 'r', encoding='cp949')
...
def f_dust(day,date_option):
    dust = open('korea_univ_python_1/2학기 기말 팀프로젝트/csv_file/dust.csv', 'r', encoding='cp949')
...
def f_tem(day, date_option):
    tem = open('korea_univ_python_1/2학기 기말 팀프로젝트/csv_file/tem.csv', 'r', encoding='cp949')
...
###################################################################################
    ...
    if avg_rain == 0:
        print("{}에는 비가오지 않을 것 같습니다.\n".format(now))
    else:
        print("{:.1f}mm의 비가 올 가능성이 있습니다.\n".format(avg_rain))
